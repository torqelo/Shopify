(() => {
  ...
  const write = (items) => { try { localStorage.setItem(KEY, JSON.stringify(items)); } catch (_) { /* Storage can be disabled by browser privacy settings. */ } };
  ...
  document.querySelectorAll('[data-recently-viewed-section]').forEach((section) => {
    ...
    if (!grid) return;
    grid.innerHTML = items.map((item) => `<article class="torqelo-recent-card"><a href="${esc(item.url)}">${item.image ? `<img src="${esc(item.image)}" alt="${esc(item.title)}" loading="lazy" width="320" height="320">` : ''}<h3>${esc(item.title)}</h3>${item.price ? `<p>${esc(item.currency)} ${esc(item.price)}</p>` : ''}</a></article>`).join('');
